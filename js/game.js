(function () {
  ...
  function moveSnake(snake) {
    ...
    if (dist(0, 0, newX, newY) + CONFIG.HEAD_RADIUS >= CONFIG.WORLD_RADIUS) {
  ...
  function drawPlayGrid(cx, cy, radius) {
    ...
    const half = Math.ceil(radius / CONFIG.GRID_SIZE) * CONFIG.GRID_SIZE;

    for (let wx = -half; wx <= half; wx += CONFIG.GRID_SIZE) {
      if (Math.abs(wx) >= radius) continue;
      const sx = cx + wx;
      const span = Math.sqrt(radius * radius - wx * wx);
      ctx.beginPath();
      ctx.moveTo(sx, cy - span);
      ctx.lineTo(sx, cy + span);
      ctx.stroke();
    }

    for (let wy = -half; wy <= half; wy += CONFIG.GRID_SIZE) {
      if (Math.abs(wy) >= radius) continue;
      const sy = cy + wy;
      const span = Math.sqrt(radius * radius - wy * wy);
      ctx.beginPath();
      ctx.moveTo(cx - span, sy);
      ctx.lineTo(cx + span, sy);
      ctx.stroke();
    }
  }

  function drawWarningZone(cx, cy, radius) {
    const warnInner = radius - CONFIG.WARN_ZONE;

    ctx.save();
    ctx.beginPath();
    ctx.arc(cx, cy, warnInner, 0, Math.PI * 2);
    ctx.arc(cx, cy, radius, 0, Math.PI * 2, true);
    ctx.clip('evenodd');

    const grad = ctx.createRadialGradient(cx, cy, warnInner, cx, cy, radius);
    grad.addColorStop(0, 'rgba(255, 60, 100, 0)');
    grad.addColorStop(0.45, 'rgba(255, 60, 100, 0.025)');
    grad.addColorStop(0.75, 'rgba(255, 80, 120, 0.07)');
    grad.addColorStop(1, 'rgba(255, 50, 90, 0.14)');
